...
# 24 items as requested (exact lowercase + underscores)
ITEM_KEYS = [
...
# ---------- DESCRIPTIONS (kept from your version) ----------
DESC = {
    ...
    "candle": """Candles symbolize Christ as the Light of the World. Early Christians used candles in catacombs; by the 4th century, they became standard in liturgy. Their origins can be traced back to Jewish practices, where a perpetual light was kept in the Temple to signify God's presence, which Christians adapted for their own rituals over time. There are several types of candles used in the Catholic tradition. Here are a few examples: The Paschal candle, used during Easter Vigil, first mentioned in the 5th century, represents the Resurrection and light of Christ. Votive candles, lit for specific prayer intentions, often placed before statues or images. Altar candles, present during Mass to signify the presence of Christ in the Eucharist.""",
...
def img_data_uri(path: Path | None, fallback: Path | None) -> str:
    # Returns data: URI for path or fallback. Empty string if neither.
...
st.markdown(f"""
<style>
/* Antique Roman / Old-English styled title via Google Fonts */
...
/* Style expanders to blend with cards */
...
for idx, key in enumerate(ITEM_KEYS):
    col = cols[idx % 3]
    with col:
        # choose item image or placeholder
        item_img = first_existing(ASSETS_DIR / key)
        use_img_uri = img_data_uri(item_img, ph_file)

        st.markdown('<div class="sac-card">', unsafe_allow_html=True)

        if use_img_uri:
            st.markdown(f'<img src="{use_img_uri}" alt="{humanize(key)}" class="sac-img"/>',
                        unsafe_allow_html=True)

        # Collapsible description (no extra on-screen instructions)
        with st.expander(humanize(key), expanded=False):
            st.write(DESC.get(key, "Description coming soon."))
