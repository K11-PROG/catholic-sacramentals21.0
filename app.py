...
Wearing a Saint Michael medal is a call for his protection against the snares of the devil and a reminder of the spiritual battle between good and evil. It is especially popular among police officers, soldiers, and those in dangerous professions who seek courage and protection in their line of duty.""",
    "blessed_salt": """Blessed salt is a sacramental in the Catholic tradition, used historically in various rites, including baptism, to symbolize purification and protection. Its origins trace back to early Christianity, where it was associated with blessings and exorcisms, as noted in biblical accounts like the healing of the waters of Jericho by the prophet Elisha (2 Kings 2:19-22).
...
Incense has been used in Catholic tradition since ancient times, symbolizing purification and the prayers of the faithful rising to God. Its origins trace back to Jewish worship practices, where it was commanded by God for use in the Tabernacle, and it has been integrated into Christian liturgy since the early Church.

The Bible mentions incense over 170 times, indicating its significance in worship. God commanded the Israelites to burn incense in the Tabernacle (Exodus 30:1-10).""",
...
How many soldiers have repeated the Hail Mary over and over on the battlefield? In our darkest hour, even the hour of our death, we plead for the intercession, the blessing and comfort of the Blessed Mother using this 700-year-old devotion which ends, in part, “Turn then most gracious advocate thine eyes of mercy toward us …”""",
    "cord": """There are several types of Catholic cords, including the wedding cord, which symbolizes the union of the couple during marriage, and the cords associated with various saints, such as the Cord of Saint Francis, the Cord of St Philomena and the Cord of Saint Joseph, which are worn for spiritual benefits and intercession. These traditions have ancient origins, often linked to the practices of early Christians and saints, and have been formalized into various confraternities within the Catholic Church. 
    
Cords symbolize various virtues, worn around the waist, they symbolize purity, fidelity, and special petitions for protection and grace.""",
...
# ---------- SIDEBAR (minimal) ----------
# Minimal layout: a small "Explore" header is omitted per request
view_mode = st.sidebar.radio("", ["Grid", "Timeline"], index=0, label_visibility="collapsed")
search_query = st.sidebar.text_input("", placeholder="Search sacramentals…").strip().lower()
selected_categories = st.sidebar.multiselect("", ALL_CATEGORIES, default=[], label_visibility="collapsed")
...
# ---------- Build URIs (for JS lightbox navigation) ----------
def uris_for_item(item_key: str):
    paths = multiple_existing(ASSETS_DIR / item_key, max_images=3)
    out = []
    for p in paths:
...
# ---------- RENDER (GRID or TIMELINE) ----------
# grid columns for Grid view; single column for Timeline
if view_mode == "Grid":
    cols = st.columns(3, gap="large")
else:
    cols = [st]  # single "column" (the Streamlit module) to write sequentially
...
for idx, key in enumerate(DISPLAY_KEYS):
    ...
    with col:
        ...
        # Selected index (for small in-card carousel)
        selected_idx = 0
        if images:
            if len(images) > 1:
                # small horizontal radio allows simple swipe-like selection
                selected_idx = st.radio(
                    "", list(range(len(images))),
                    horizontal=True, label_visibility="collapsed", key=f"radio_{key}"
                )
            ...
            img_uri = img_data_uri(hires_path, ph_file)
            tooltip = FACTS.get(key, humanize(key))
            # data-index corresponds to the index within the group's URIs
            st.markdown(
                f'<img src="{img_uri}" alt="{humanize(key)}" class="sac-img" '
        ...
        # Minimal view button (just an eye icon) that opens the JS lightbox for the whole group
        # We'll render a tiny button that calls the JS function (bound later) via a special anchor with data attributes.
        st.markdown(
            f'''
            <div style="margin-top:8px;">
              <a class="lb-open-js" href="javascript:void(0)" data-group="{key}" title="View gallery"
                 style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.06);padding:6px 10px;border-radius:10px;border:1px solid rgba(255,255,255,0.12);text-decoration:none;color:#fff;">
        ...
        # Collapsible description — break into paragraphs (already prepared)
        with st.expander(humanize(key), expanded=False):
            text = DESC.get(key, "Description coming soon.")
            for para in text.split("\n"):
                if para.strip():
...
# ---------- GLOBAL JS LIGHTBOX (for full gallery modal + swipe + keyboard) ----------
# We pass the `all_item_uris` mapping to JS so the gallery can show all images per group.
images_json = json.dumps(all_item_uris)

# The HTML below injects a lightbox overlay and binds click events:
components.html(f"""
<div id="global-lightbox-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.88); z-index:10000; align-items:center; justify-content:center;">
  <div id="global-lightbox-inner" style="position:relative; max-width:96vw; max-height:90vh; display:flex; align-items:center; justify-content:center;">
    <button id="glb-prev" style="position:absolute; left:-56px; top:50%; transform:translateY(-50%); font-size:28px; background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.18); color:#fff; padding:8px 12px; border-radius:10px;">‹</button>
    <img id="global-lightbox-img" src="" style="max-width:96vw; max-height:90vh; border-radius:10px; box-shadow:0 10px 40px rgba(0,0,0,0.6);" />
    <button id="glb-next" style="position:absolute; right:-56px; top:50%; transform:translateY(-50%); font-size:28px; background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.18); color:#fff; padding:8px 12px; border-radius:10px;">›</button>
    <button id="glb-close" style="position:absolute; top:-56px; right:0; font-size:20px; background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.18); color:#fff; padding:6px 10px; border-radius:8px;">✕</button>
  </div>
</div>
...
(function(){
  ...
  const btnNext = document.getElementById('glb-next');
  const btnPrev = document.getElementById('glb-prev');
  const btnClose = document.getElementById('glb-close');
  ...
  // bind gallery-open anchors (the small eye icon)
  function bindOpeners(){
    const opens = document.querySelectorAll('a.lb-open-js');
    opens.forEach(el => {
      el.onclick = (e) => {
        e.preventDefault();
        const group = el.getAttribute('data-group');
  ...
  // bind clicking on a sac-img to open lightbox at that image index
  function bindImages(){
    const imgs = document.querySelectorAll('img.sac-img');
    imgs.forEach((el) => {
      el.onclick = (e) => {
      ...
      // Title tooltip already comes from title attribute
    });
  }
  ...
  // initial bind + mutation observer to rebind after Streamlit rerenders
  bindOpeners();
  bindImages();
  const mo = new MutationObserver(() => { bindOpeners(); bindImages(); });
  mo.observe(document.body, { childList: true, subtree: true });
})();
</script>
""", height=10)
...
# ---------- FOOTER INFO (minimal) ----------
st.markdown("---")
st.caption("Tip: drop images into the `assets/` folder using exact filenames (e.g., `rosary.jpg`, `rosary_1.jpg`, `rosary_2.jpg`). A `background.*` and `placeholder.*` in `assets/` are recommended.")
